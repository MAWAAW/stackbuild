...
create_github_repo() {
    ...
    print_info "Création du dépôt GitHub..."
    ...
    print_info "Création du dépôt $project_name..."
    ...
    if gh repo create "$project_name" \
        ...
        print_info "URL du dépôt: $repo_url"
...
setup_github_secrets() {
    ...
    echo ""
    print_info "Pour automatiser les déploiements, nous avons besoin de:"
    echo "  1. RENDER_API_KEY - Pour déployer le backend"
    echo "  2. NETLIFY_AUTH_TOKEN - Pour déployer le frontend"
    echo ""
    ...
    read -p "   Entrez votre RENDER_API_KEY (ou appuyez sur Entrée pour ignorer): " render_key
    ...
    read -p "   Entrez votre NETLIFY_AUTH_TOKEN (ou appuyez sur Entrée pour ignorer): " netlify_token
    ...
    print_success "Configuration des secrets terminée"
...
📋 Création manuelle du dépôt GitHub:
...
Option B - Manuellement:
  ...
  3. Ne créez pas de README, .gitignore ou LICENSE
  4. Créez le repo
...
check_git_status() {
    ...
    if [ -d ".git" ]; then
        ...
        if git remote get-url origin &> /dev/null; then
            ...
            print_success "Remote configuré: $remote"
        else
            print_warning "Aucun remote configuré"
        fi
    else
        print_warning "Pas de dépôt Git"
    fi
    
    cd - > /dev/null
}

# Fonction pour vérifier et installer gh CLI si nécessaire
install_gh_cli() {
    if command -v gh &> /dev/null; then
        print_success "GitHub CLI déjà installé"
        return 0
    fi
    ...
    case $os in
        ...
        Linux)
            ...
            if command -v apt-get &> /dev/null; then
                ...
                print_error "Distribution non supportée. Installez manuellement: https://cli.github.com"
                return 1
            fi
            ;;
        *)
            print_error "OS non supporté. Installez manuellement: https://cli.github.com"
            return 1
            ;;
    esac
}
